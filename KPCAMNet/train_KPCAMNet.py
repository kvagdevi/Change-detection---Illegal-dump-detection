...
import sys
from KPCAMNet.KPCANet import KernelPCANet
from KPCAMNet.cluster_util import get_binary_change_map
from KPCAMNet.acc_ass import assess_accuracy
from KPCAMNet.data_pro_util import stad_img, norm_img, random_select_samples
import imageio
import fiona
import rasterio
...
def load_data(image_path, image_path2):

    # data_set_X = image_path # rasterio.open(image_path, driver='jp2openjpeg')
    #
    # data_set_Y = image_path2 #rasterio.open(image_path2, driver='jp2openjpeg')
    # aoiFile = fiona.open(
    #     '/opt/users/ckarr01/Downloads/ffff.shp')  # for crop the image based on shapefile
    # aoiGeom = [aoiFile[0]['geometry']]
    # img_X, outTransform = mask(data_set_X, aoiGeom, crop=True)  # crop the image
    # img_Y, outTransform = mask(data_set_Y, aoiGeom, crop=True)  # crop the image
    img_X = image_path # img_X[:, 120:140, 120:140]
    img_Y = image_path2 #img_X[:, 120:140, 120:140]
...
def perform_KPCAMNet(image_path, image_path2):
    img_X, img_Y = load_data(image_path, image_path2)
    height, width, channel = img_X.shape
    NET_DEPTH = 3
    SAMPLE_NUM = 100
    PATCH_SZ = 5
    FILTER_NUM = [8, 8, 8]
    GAMMA = [5e-4, 5e-4, 5e-4]
    KERNEL_FUNC = 'rbf'
    ...
    SAVE_PATH = '/opt/users/ckarr01/PycharmProjects/pipeline/KPCAMNet/result/'
    ...
    return binary_change_map, multi_change_map
